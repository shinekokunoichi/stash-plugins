...
sk.tool = function skTool() {
    ...
    */
    ...
    /**
     * Create the toast for notification
     */

    const _toast = () => {
        const toast = sk.ui.make.popUp({ id: 'skToastNotification', style: { 'text-align': 'left', bottom: 0, right: '-100%', padding: '1rem 3rem', width: 'auto', height: 'auto', 'border-radius': '1rem', transition: 'all 1s ease-out' } });
        const title = sk.ui.make.subTitle();
        const message = sk.ui.make.description();
        toast.append([title, message]);
        document.body.append(toast.element);
    };

    let _pendingNotification = [];

    /**
     * Create a toast notification
     * @param {string} [title=''] Title of the notification
     * @param {string} [message=''] Message of the notification
     * @param {string} [type='General'] General|Success|Error
     */
    const notify = (title = '', message = '', type = 'General') => {
        if (!sk.tool.get('#skToastNotification')) _toast();
        const toast = sk.tool.get('#skToastNotification');
        if (toast.style('right') !== '-100%') {
            _pendingNotification.push({ title: title, message: message, type: type });
            return;
        };
        const [notificationTitle, notificationMessage] = toast.child();
        if (type.toLowerCase() === 'general') toast.style({ 'background-color': 'rgba(0,0,0,.25)' });
        if (type.toLowerCase() === 'success') toast.style({ 'background-color': 'darkgreen' });
        if (type.toLowerCase() === 'error') toast.style({ 'background-color': 'darkred' });
        notificationTitle.write(title);
        notificationMessage.write(message);
        toast.style({ right: 0 });
        setTimeout(() => {
            toast.style({ right: '-100%' });
            if (_pendingNotification[0]) setTimeout(() => {
                notify(_pendingNotification[0].title, _pendingNotification[0].message, _pendingNotification[0].type);
                _pendingNotification.shift();
            }, 500);
        }, 5000);
    };
    ...
    const getAll = (selector, searchIn = document) => {
        ...
        for (el of list) { elements.push(sk.element(el)) };
        return elements;
    };
    ...
    const get = (selector, searchIn = document) => {
        ...
        return sk.element(searchIn.querySelector(selector));
    };
    ...
    const scrape = async (url, options) => {
        ...
        if (options.html) {
            ...
            return sk.element(parser.parseFromString(data, 'text/html'));
        };
    };
...
sk.element = (tag, options = {}) => {
     ...
     * @returns {skUI[]} Selected elements
     */
    base.getAll = selector => sk.tool.getAll(selector, base.element);
     ...
     * @returns {skUI} Selected element
     */
    base.get = selector => sk.tool.get(selector, base.element);
     ...
     */
    base.id = id => id ? base.element.id = id : base.element.id;
     ...
     */
    base.read = html => html ? base.element.innerHTML : base.element.innerText
     ...
     * @return {string} Value
     */
    base.value = value => value ? base.element.value = value : base.element.value;
    ...
    base.child = () => {
        let children = [];
        for (el of base.element.children) children.push(sk.element(el));
        return children;
    };

    /**
     * Append children (skUI or HTML element)
     * @param {...HTMLElement|...skUI} chilren Childrens to append
     */
    base.append = (...children) => Array.isArray(children[0]) ? children[0].forEach(child => child.element ? base.element.appendChild(child.element) : base.element.appendChild(child)) : children.forEach(child => child.element ? base.element.appendChild(child.element) : base.element.appendChild(child));

    /**
     * Add events to the element
     * @param {...object} eventList Events to add
     * @param {string} eventList.type Event type
     * @param {function} eventList.callback Function to execute
     */
    base.event = (...eventsList) => Array.isArray(eventsList[0]) ? eventsList[0].forEach((ev) => { base.element.addEventListener(ev.type, ev.callback); }) : eventsList.forEach((ev) => { base.element.addEventListener(ev.type, ev.callback); });
...
sk.ui = function skGraphic() {
     ...
     * @prop {skUI} tagsCount Tags popover count
    ...
    const get = {
        ...
        */
        ...
        cards: {
            ...
            /**
             * @typedef {skUI & {_data: skBaseCard & _skUICardScene}} skUICardScene
             */
            ...
            /**
             * @typedef {skUI & {_data: skBaseCard & _skUICardImage}} skUICardImage
             */
            ...
            /**
             * @typedef {skUI & {_data: skBaseCard & _skUICardGroup}} skUICardGroup
             */
            ...
            /**
             * @typedef {skUI & {_data: skBaseCard & _skUICardMarker}} skUICardMarker
             */
            ...
            /**
             * @typedef {skUI & {_data: skBaseCard & _skUICardGallery}} skUICardGallery
             */
            ...
            /**
             * @typedef {skUI & {_data: skBaseCard & _skUICardPerformer}} skUICardPerformer
             */
            ...
            /**
             * @typedef {skUI & {_data: skBaseCard & _skUICardStudio}} skUICardStudio
             */
            ...
            /**
             * @typedef {skUI & {_data: skBaseCard & _skUICardTag}} skUICardTag
             */
    ...
    const make = {
        ...
        popUp: (options = {}) => {
            ...
            style['z-index'] = style['z-index'] || 1000;
            style['text-align'] = style['text-align'] || 'center';
            options.style = style;
            return sk.element('div', options);
        },
        /**
         * Create a container
         * @param {skUIOptions} [options={}] skUI options
         ...
         */
        container: (options = {}) => { return sk.element('div', options) },
         ...
         */
        divider: (options = {}) => { return sk.element('hr', options) },
         ...
         * @returns {skUI} Title
         */
        title: (options = {}) => { return sk.element('h1', options) },
         ...
         */
        subTitle: (options = {}) => { return sk.element('h3', options) },
         ...
         * @returns {skUI} Span
         */
        span: (options = {}) => { return sk.element('span', options) },
         ...
         */
        description: (options = {}) => { return sk.element('p', options) },
         ...
         */
        li: (options = {}) => { return sk.element('li', options) },
        ...
        ulist: (options = {}, li = [], liOptions = {}) => {
            const ul = sk.element('ul', options);
            li.forEach((item) => {
                liOptions.text = item;
                ul.append(sk.element('li', liOptions));
            });
            return ul;
        },
         ...
         * @returns {skUI} Anchor
         */
        link: (options = {}) => { return sk.element('a', options) },
         ...
         * @returns {skUI} Image
         */
        image: (options = {}) => { return sk.element('img', options) },
         ...
         * @returns {skUI} Video
         */
        video: (options = {}) => { return sk.element('video', options) },
         ...
         * @returns {skUI} Button
         */
        button: (options = {}) => { return sk.element('button', options) },
        ...
        checkBox: (options = {}) => {
            ...
            return sk.element('input', options);
        },
         ...
         * @returns {skUI} Input
         */
        input: (options = {}) => { return sk.element('input', options) }
    };
...
sk.stashDB = function skStashDB() {
    ...
    const _get = {
        ...
        urls: (urls) => {
            let stashUrls = [];
            urls.forEach((url) => { stashUrls.push(url.url) });
            return stashUrls;
        },
        /**
    ...
    const _toStash = {
        ...
        performer: async (data, options = {}) => {
            ...
            if (data.birth_date) performer.birthdate = data.birth_date
