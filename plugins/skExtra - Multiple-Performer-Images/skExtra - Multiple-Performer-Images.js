                ...
                type: 'click', callback: async () => {
                    ...
                    names.concat(['remove', 'close']).forEach((type) => {
                        ...
                        const button = sk.ui.make.button({
                            id: `skMPIS-${type}`, text: type, class: `btn btn-${buttonType}`, style: { margin: '.5%' }, event: {
                                type: 'click', callback: () => {
                            ...
                            style: { cursor: 'pointer', background: `url(${thumbnail.paths.thumbnail}) no-repeat top center`, 'background-size': 'cover', 'min-width': '150px', 'min-height': '150px' }, event: {
                                type: 'click', callback: async () => {
                                    ...
                                    if (selected !== 'remove') {
                                        ...
                                        sk.tool.get(`#skMPIS-${uSelected}`).class('btn btn-primary', true);
                                    };
                                    //Remove the category
                                    if (selected === 'remove') {
                                        ...
                                        sk.tool.get(`#skMPIS-${toRemove}`).class('btn btn-secondary', true);
                                        newSettedText.pop();
